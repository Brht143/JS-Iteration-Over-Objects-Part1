...
********************************/
person.email = "i.h.chami@gmail.com"
...
const movies = [
  ...
  ********************************/
  ...
  const getTitles = array => array.map(movie => movie.title);
  ...
  ********************************/
  ...
  const countMovies = array => array.reduce((total, movie) => total += 1,0);
  const moviesTotal = countMovies(movies);
  ...
  ********************************/
  ...
  const updateGenre = (array, title, genre) => array.forEach(movie => {if (movie.title === title) return movie.genre = genre});
  const GenreModify = updateGenre(movies, "The Dark Knight", "Action/Drama")
//   console.log(movies);